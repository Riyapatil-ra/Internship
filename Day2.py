...
''' Naming Variables'''
# if same variable name is used inside and outside the function, 
# then inside the function it will be treated as local variable and outside it will be treated as global variable.
# x=300
# def demo():
#     x=200
#     print(x)
...
* E: Enclosing  - Inside enclosing functions (from inner to outer)
...
then apply it with @decorator_name above the function.
'''
# def changecase(func):
#   def myinner():
#     return func().upper()
...
''' Arguments in the decorator
* If you want to pass arguments to the decorator,
* you need to add another layer of functions.
'''
# def changecase(func):
#   def myinner(x):
#     return func(x).upper()
...
''' Prserving Function Metadata 
* Functions in Python has metadata that can be accessed using the
...
* Lambda functions are used when you need a nameless function for a short period of time.
* They are often used as arguments to higher-order functions, which take other functions as arguments.
'''
# def demo(n):
#     return lambda a:a*n
